...
def pid_ct(p, bw, pm, plot=False, maxpole=False):
    zeros = -np.roots(p.num[0][0])                  # really the negative of the zeros & poles
    poles = -np.roots(p.den[0][0])
    ...
    theta = -np.pi + np.radians(pm) + np.pi/2 + np.arctan(tau * wc)
    alpha = beta = 0
    for z in zeros:
